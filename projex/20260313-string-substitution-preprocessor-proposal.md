# Standard String Substitution Preprocessor (`#sub`)

> **Status:** Draft
> **Created:** 2026-03-13
> **Author:** Agent
> **Related Projex:** 20260312-locale-companion-file-explore.md, 20260312-script-level-localization-proposal.md, 20260313-embed-variable-locale-flag-proposal.md, 20260313-locator-preprocess-phase-explore.md, 20260313-runtime-scoped-flags-proposal.md

---

## Summary

A new `#sub` preprocessor directive for compile-time string literal substitution. `#sub` defines a source-to-replacement mapping that the preprocessor applies to all matching string literals in the story source. Companion files containing `#sub` directives are delivered via `#embed` (or `#embed?` for optional delivery). This becomes the fourth standard preprocessor in the pipeline, alongside embed, macro, and sugar. Primary use case: localization. Generalizes to content variants, terminology adaptation, and any scenario requiring bulk string substitution before compilation.
...
```zoh
#sub "source string" -> "replacement string";
```

A preprocessor directive that registers a string-to-string substitution. The preprocessor collects all `#sub` directives, removes them from the source, then applies the registered substitutions to all matching string literals in the remaining source text.

**Syntax:**
- Starts with `#sub` at the beginning of a line (leading whitespace allowed)
...
```zoh
:: Single-line form
#sub "The cafe closes at midnight." -> "Le cafe ferme a minuit.";
#sub 'Sit across from her' -> "S'asseoir en face d'elle";

:: Multi-line form (natural for long strings)
#sub "The cafe closes at midnight."
...
**Why after embed:** `#sub` directives arrive via `#embed` from companion files. They must be inlined before collection.
...
SubPreprocessor.process(source):
    1. COLLECT: Scan source for all #sub directives.
    ...
    4. REPORT: Emit diagnostics for #sub entries that matched nothing
...
**Matching semantics:**
- Match is on **evaluated string content**, quote-style agnostic: `'Hello'` and `"Hello"` match the same `#sub` key
...
A companion file is a plain file containing `#sub` directives:

```zoh
:: cafe.fr.zoh -- French translations for The Last Coffee Shop

#sub "11:47 PM. The cafe closes at midnight."
-> "23h47. Le cafe ferme a minuit.";

#sub "A woman in a red coat sits alone by the window."
-> "Une femme en manteau rouge est assise seule pres de la fenetre.";

#sub "The rain picks up outside." -> "La pluie s'intensifie dehors.";
#sub "Sit across from her" -> "S'asseoir en face d'elle";
#sub "Order something first" -> "Commander quelque chose d'abord";
#sub "Leave" -> "Partir";
#sub "Narrator" -> "Narrateur";
```
...
When `locale` is `"fr"`: resolves to `#embed? "cafe.fr.zoh"` -> inlines the `#sub` directives -> SubPreprocessor applies them -> all matching string literals are replaced before compilation.
...
All string literals in the story body that exactly match a `#sub` key, regardless of position. This includes:
...
- **Preprocessor directive arguments:** `#embed "path.zoh"` -- the embed path is consumed at priority 100, before `#sub` runs at 250. Even if it weren't, directive arguments are not story body strings.
- **`#sub` directive strings:** The `#sub` directives themselves are collected and removed before replacement begins. A `#sub` source key does not match against other `#sub` entries.
...
- **Macro definitions:** Macro bodies are already expanded by priority 200. The definitions no longer exist in the source when `#sub` runs.
...
```zoh
#sub "Hello, ${*name}. Welcome back." -> "Bonjour, ${*name}. Bon retour.";
```
...
Replace every string literal whose content matches a `#sub` key, regardless of its syntactic position (verb parameter, attribute value, sugar form, etc.).
...
| `#sub` naming | Concise, evocative of "substitute" (Unix `sed s///`). Alternatives: `#replace` (verbose), `#swap` (informal), `#tr` (cryptic). |
...
| No regex support | Exact match only. Regex introduces complexity, security concerns, and hard-to-debug behavior. If pattern-based replacement is needed, a custom preprocessor is the right tool. |

### Why Not Just `/patch`?

| | `#sub` (preprocessor) | `/patch` (runtime) |
|---|---|---|
...
- `#sub` handles the static bulk case (the 95% of localization)
- `/patch` handles dynamic, conditional, and structurally-addressed cases (the 5%)

A project can use either, both, or neither. Using both: `#sub` for base translations, `/patch` for runtime overrides or context-specific adjustments.
...
- **Spec: `1_concepts.md`** -- New `#sub` directive in the Embed/Macro/Sugar section
- **Impl: `03_preprocessor.md`** -- SubPreprocessor implementation (priority 250)

### Dependencies

- **`#embed?` and variable interpolation** (20260313-embed-variable-locale-flag-proposal.md) -- delivery mechanism for companion files. `#sub` works without it (manual `#embed` or inline directives), but `#embed?` enables the clean `"${filename}.${locale}.zoh"` pattern.
- **Runtime-scoped `locale` flag** (20260313-runtime-scoped-flags-proposal.md) -- provides the `${locale}` value for `#embed?` path resolution. Again, `#sub` works without it, but the flag enables automatic locale file selection.
...
| Stale companion file after script edits | Medium | Low | Compile-time warning on unmatched `#sub` keys |
...
None. Fully additive. Existing scripts without `#sub` directives behave identically. No syntax is repurposed.
...
:: French translations
#sub "Narrator" -> "Narrateur";
#sub "11:47 PM. The cafe closes at midnight." -> "23h47. Le cafe ferme a minuit.";
#sub "A woman in a red coat sits alone by the window."
-> "Une femme en manteau rouge est assise seule pres de la fenetre.";
#sub "The rain picks up outside." -> "La pluie s'intensifie dehors.";
#sub "Sit across from her" -> "S'asseoir en face d'elle";
...
**`variants/deluxe.zoh`:**
```zoh
#sub "Welcome to the standard edition."
-> "Welcome to the deluxe edition. Bonus content awaits.";
```

### Combined with `/patch`

Static translations via `#sub`, runtime refinement via `/patch`:
...
Here `#sub` handles the base French translation. `/patch` conditionally overrides one specific string with a formal register variant at runtime.

---

## Open Questions

- [ ] Should `#sub` support an optional key/tag for disambiguation (e.g., `#sub [key:"flashback"] "source" -> "replacement";`)? Or is disambiguation exclusively `/patch`'s domain?
- [ ] Should unmatched `#sub` keys produce a warning (recommended) or an error? Warnings allow incremental translation; errors enforce completeness.
- [ ] Should `#sub` skip strings inside `#embed` path arguments? (Likely yes -- embed paths are consumed at priority 100, before `#sub` runs. But if a future preprocessor at priority < 250 introduces path-like strings, the boundary matters.)
- [ ] Naming: `#sub` vs `#replace` vs `#substitute`? `#sub` is concise; `#replace` is more self-documenting.
...
- [ ] Should `#sub` support a `?` (optional) modifier like `#embed?`? e.g., `#sub? "text" -> "other";` that silently no-ops if the source string isn't found. This would suppress the stale-key diagnostic for intentionally optional entries.
...
If accepted:
1. Spec: Add `#sub` directive to `1_concepts.md` alongside `#embed` and macro
2. Impl: Add SubPreprocessor to `03_preprocessor.md` at priority 250
3. Update `/patch` proposal to reference `#sub` as the complementary static mechanism
4. Update `#embed?` proposal to reference `#sub` as the primary consumer of locale companion files
...
| System | Mechanism | Relation to `#sub` |
|--------|-----------|-------------------|
| GNU gettext | `msgid`/`msgstr` in `.po` files, compiled to `.mo` | Same source-as-key philosophy. gettext operates at runtime; `#sub` at compile time. |
| C/C++ `#define` | Preprocessor text substitution | `#sub` is string-literal-aware (not raw text), avoiding macro pitfalls. |
| Ren'Py `translate` | Block-structured translation tied to labels | More tightly coupled to script structure. `#sub` is position-independent. |
| Android `strings.xml` | Key-based string resources | Key indirection model. `#sub` uses source-as-key (no IDs to manage). |
| iOS `.strings` / `.stringsdict` | `"key" = "value";` format | Similar file format. `#sub` integrates as a preprocessor directive, not an external resource system. |

### Alternatives Considered

- **Preprocessor-only localization (20260312-locale-companion-file-explore.md):** Explored a verb-context-aware preprocessor with a dedicated companion file format (`"source" = "translation";` in a story body). This proposal simplifies: `#sub` is a directive, blanket replacement, no verb-awareness needed. The companion file is just a list of `#sub` directives.

- **Runtime-only via `/patch`:** Sufficient but suboptimal for the static case. `/patch` scans compiled data at execution time for every locale load. `#sub` resolves at compile time, once.

- **Dedicated locale file format:** A story-like file with `locale:` metadata and `"source" = "translation";` body entries. Rejected: introduces new body syntax and a format that exists only for localization. `#sub` directives are general-purpose and reuse the existing preprocessor directive model.
